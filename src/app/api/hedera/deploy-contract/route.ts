...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!requirements) {
      ...
      return NextResponse.json({ error: "Paramètres de contrat manquants" }, { status: 400 })
    }

    // 1. Générer le smart contract avec NFT intégré
    console.log("🧠 Génération du contrat avec NFT...")
    const contractCode = await aiContractGenerator.generateSmartContract(requirements)
    ...
    const nftMetadata = aiContractGenerator.generateNFTMetadata(requirements, mockContractId)

    // 4. Simuler la création du NFT
    const nftTokenId = Math.floor(Math.random() * 10000) + 1
    const nftImageUrl = `https://agrisure-nft.vercel.app/api/generate-nft/${mockContractId}?type=${requirements.insuranceType.toLowerCase().replace(/\s+/g, "-")}&coverage=${requirements.coverageAmount}`
    ...
    return NextResponse.json(
      {
        ...
        timestamp: new Date().toISOString(),
        suggestion: "Le contrat utilise maintenant un template NFT prédéfini. Réessayez.",
      },
      { status: 500 },
    )
  }
