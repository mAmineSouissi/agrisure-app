...
import { Send, Bot, User, Leaf, Cloud, BarChart3, Lightbulb } from "lucide-react"
...
export function ChatbotPage() {
  ...
  const generateBotResponse = (userInput: string): Message => {
    ...
    if (input.includes("sécheresse") || input.includes("risque")) {
      ...
      suggestions = ["Comment activer l'irrigation ?", "Quand sera versé le paiement ?", "Autres risques climatiques ?"]
      ...
      suggestions = ["Où acheter ces semences ?", "Calendrier de plantation ?", "Besoins en irrigation ?"]
      ...
      suggestions = ["Comment fonctionne Hedera ?", "Délais de paiement ?", "Augmenter ma couverture ?"]
      ...
      suggestions = ["Quels événements sont couverts ?", "Comment modifier ma police ?", "Contacter le support ?"]
    } else {
      response =
                ...
                <div className={`max-w-[80%] ${message.type === "user" ? "order-first" : ""}`}>
                  <div
                    className={`p-3 rounded-lg ${
                      message.type === "user" ? "bg-green-600 text-white ml-auto" : "bg-muted"
                    }`}
                  >
        ...
        <CardContent className="p-4">
          ...
          >
            <Input
              ...
              disabled={isTyping}
            />
            <Button type="submit" disabled={!inputValue.trim() || isTyping} className="bg-green-600 hover:bg-green-700">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <Button
              ...
              size="sm"
              className="text-xs h-7"
              onClick={() => handleSendMessage("Quel est le statut de mes cultures ?")}
            >
            ...
            <Button
              ...
              size="sm"
              className="text-xs h-7"
              onClick={() => handleSendMessage("Prévisions météo cette semaine")}
            >
